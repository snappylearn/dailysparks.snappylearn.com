...
async function main() {
  console.log("\n🧬 Biology Content Generation for Forms 1-4");
  console.log("=" + "=".repeat(60) + "\n");

  // Get Biology subject
  const [bioSubject] = await db
    .select()
    .from(subjects)
  ...
  // Get all Biology topics without content, ordered by level
  const bioTopics = await db
    .select({
      id: topics.id,
      title: topics.title,
    ...
    .where(
      and(
        eq(topics.subjectId, bioSubject.id),
        or(
  ...
  console.log(`📋 Found ${bioTopics.length} Biology topics to process\n`);

  if (bioTopics.length === 0) {
    console.log("✅ All Biology topics already have content!");
    process.exit(0);
  }
  ...
  for (let i = 0; i < bioTopics.length; i++) {
    const topic = bioTopics[i];
    console.log(`\n[${i + 1}/${bioTopics.length}] ${topic.levelTitle} - ${topic.title}`);
  ...
  console.log("\n" + "=".repeat(60));
  console.log("📊 Biology Content Generation Summary");
  console.log("=".repeat(60));
  console.log(`✅ Completed: ${completed}`);
  console.log(`❌ Failed: ${failed}`);
  console.log(`📚 Total: ${bioTopics.length}`);
  console.log("=".repeat(60) + "\n");
}
